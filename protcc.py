...
from utils.ss_process import DSSP
from utils.pdb_cleaning import fix_pdb
from utils.pocket_process import FPocket
...
THREE2ONE = {v: k for k, v in RESIDUES.items()}
...
class ProteinCombinatorialComplexDataset(InMemoryDataset):
    ...
    def download(self) -> None:
        ...
        for pdb_id in tqdm(pdb_ids, desc="Downloading PDB files"):
            try:
                downloaded_path = pdbl.retrieve_pdb_file(pdb_id, pdir=self.pdb_dir,
                                                         file_format='pdb')
    ...
    def process(self, tmp_dir: Optional[str] = 'tmp',
                clean_pdbs: Optional[bool] = False,
                ss_dict: Optional[int] = 8,
                spatial_distance_threshold: Optional[float] = 8.0) -> None:
        data_list = []
        dssp = DSSP()
        fpocket = FPocket()
        pdb_files = [f for f in self.raw_file_names[1:]]
        os.makedirs(tmp_dir, exist_ok=True)
        for pdb_file in tqdm(pdb_files, desc="Processing PDB files"):
            pdb_path = osp.join(self.pdb_dir, pdb_file)
            tmp_path = os.path.join(tmp_dir, os.path.basename(pdb_file))
            pdb_df = PandasPdb().read_pdb(pdb_path)
            pdb_df.to_pdb(tmp_path, records=['ATOM'])
            if clean_pdbs:
                tmp_path = fix_pdb(tmp_path)

            pdb_df = PandasPdb().read_pdb(tmp_path).df['ATOM']
            # Assign secondary structure using DSSP

            residues = [c if isinstance(c, str) else 'X' for c in pdb_df[
                pdb_df["atom_name"] == 'CA'].residue_name.map(THREE2ONE)]
            ss_labels = dssp.get_ss(tmp_path, ss_dict)
            pocket_labels, pocket_properties = fpocket.get_pockets(
                pdb_path, pdb_df, tmp_dir, 0.
            )

            coords = [[r.x_coord, r.y_coord, r.z_coord] for _, r in pdb_df[
                pdb_df["atom_name"] == 'CA'].iterrows()
            ]
            ...
            threshold = spatial_distance_threshold
